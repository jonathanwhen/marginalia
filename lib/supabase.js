...
export async function getMyShares() {
  ...
  const session = await getSession();
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/shared_pages?user_id=eq.${session.user.id}&select=id,share_code,title,page_key,created_at,updated_at&order=updated_at.desc`,
    {
      headers: {
        'apikey': SUPABASE_ANON_KEY,
        'Authorization': `Bearer ${token}`
...
// Points to the extension's shared.html viewer page.
export function getShareUrl(shareCode) {
  // When hosted on GitHub Pages, this would be e.g. https://username.github.io/marginalia/shared?code=abc123
  // For now, use the extension's own shared.html page
