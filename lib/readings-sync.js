...
export async function mergeReadings(local, remote) {
  ...
  // Merge readings: last-write-wins, but as a FIELD-LEVEL OVERLAY rather
  // than a wholesale replacement. If remote came from a device that didn't
  // have e.g. conversationUrl, we don't want it to nuke that field on every
  // other device. Only fields that appear in `data` override local; fields
  // missing from `data` keep their local value.
  //
  // To intentionally clear a field, a client should send it as `null` — the
  // null IS present in `data` and will overlay correctly.
  for (const [pageKey, remoteReading] of Object.entries(remote.readings)) {
    ...
    if (!localReading) {
      ...
      if (remoteTime > localTime) {
        const { _remoteUpdatedAt, ...data } = remoteReading;
        mergedReadings[pageKey] = { ...localReading, ...data };
        changed = true;
      }
    }
  }
